...
Power management is by a separate [watchdog circuit](TCAN_Soil_Sense_circuit.png) using a ATTiny85 microcontroller and P-channel MOSFET switch:
built on a small [stripboard breakout](tinyWatchdogVoltageSwitchV3.pdf) and powered using 2x 18650 lithium ion batteries in series (~1500 mAh).

The Arduino framework was used to develop the [firmware for the board](esp32_BLE_Soil_SenseV4/esp32_BLE_Soil_SenseV4.ino) and [firmware for the watchdog timer](ATTiny85_Watchdog_Soil_SenseV2/ATTiny85_Watchdog_Soil_SenseV2.ino).  The project libraries are in separate directories and compiled using [Arduino IDE](https://www.arduino.cc/en/software/):
